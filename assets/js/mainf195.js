document.addEventListener("DOMContentLoaded", function () {
    ...
    function setActiveItem(menuItem, setNav2ndLink) {
        ...
        if (setNav2ndLink) {
            setNav2ndLink.classList.add('active');
            console.log(setNav2ndLink);

        }
    }

    if (menuItems.length > 0) {
        setActiveItem(menuItems[0], menuItems[0].closest('.main-dropdown-menu__sub').previousElementSibling);
    }
    ...
    // Rating select
    const yourRating = document.querySelectorAll('#your-rating .star');
    ...
    document.addEventListener('mousemove', (e) => {
        if (modalRegisterForm && modalLoginForm && body && bodySearch) {
            ...
            const isOutsideSearchPopup = searchPopup && !searchPopup.contains(e.target) && !searchIcon.contains(e.target);
            const isOutsideCourseCategoriesPopup = courseCategoriesPopup && !courseCategoriesPopup.contains(e.target) && !showCategoriesButton.contains(e.target);
